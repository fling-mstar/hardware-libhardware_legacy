...
void AudioPolicyManagerBase::setForceUse(AudioSystem::force_use usage, AudioSystem::forced_config config)
{
    ...
    case AudioSystem::FOR_RECORD:
        if (config != AudioSystem::FORCE_BT_SCO && config != AudioSystem::FORCE_WIRED_ACCESSORY &&
            ...
            config != AudioSystem::FORCE_BT_MIC && config != AudioSystem::FORCE_BT_A2DP
...
// Two mm play support speaker and line out
        ...
        setStreamVolumeIndex(AudioSystem::MUSIC_SUB,mStreams[AudioSystem::MUSIC_SUB].mIndexMax,newDevice);
#endif
// MStar Android Patch End
...
static void* DetectHALLoad_thread(void *input)
{
    AudioPolicyManagerBase* apm = reinterpret_cast<AudioPolicyManagerBase*>(input);
    char bt_driver[PROPERTY_VALUE_MAX] = {'\0'};
    while (!(property_get("mstar.bt.driver",bt_driver, NULL) > 0)) {
        usleep(10000);
    }
    apm->reLoadHwModule();
    return 0;
...
// MStar Android Patch Begin
                ...
                outputDesc->mDevice = ((mDefaultOutputDevice| AUDIO_DEVICE_OUT_LINE_OUT) &
...
// Two mm play support speaker and line out
#if (ENABLE_DUALMONO || ENABLE_DUAL_OUTPUT)
    initStreamVolume(AudioSystem::MUSIC_SUB,0,15);
#endif
// MStar Android Patch End
    updateDevicesAndOutputs();
...
// MStar Android Patch Begin
// refine bluetooth flow
void AudioPolicyManagerBase::reLoadHwModule()
{
    ALOGE("reLoadHwModule!");
    for (size_t i = 0; i < mHwModules.size(); i++) {
        if (0==mHwModules[i]->mHandle) {
            ALOGW("reopen HW module %s", mHwModules[i]->mName);
            mHwModules[i]->mHandle = mpClientInterface->loadHwModule(mHwModules[i]->mName);
            if (mHwModules[i]->mHandle == 0) {
                ALOGW("could not open HW module %s", mHwModules[i]->mName);
                continue;
...
status_t AudioPolicyManagerBase::initCheck()
{
    ...
    // MStar Android Patch Begin
    // refine bluetooth flow
    if (pthread_create(&detecthalso_thread_id,0,DetectHALLoad_thread,this)==0) {
        ALOGV("create detecthalso thread ok!");
    }
    // MStar Android Patch End
...
void AudioPolicyManagerBase::checkOutputForStrategy(routing_strategy strategy)
{
    ...
    if (!vectorsEqual(srcOutputs,dstOutputs)) {
        ...
        for (size_t i = 0; i < srcOutputs.size(); i++) {
            ...
            if (desc->isStrategyActive(strategy) != 0 && oldDevice != newDevice) {
...
// Two mm play support speaker and line out
#if ENABLE_DUAL_OUTPUT
    checkOutputForStrategy(STRATEGY_MEDIA_SUB);
#endif
// MStar Android Patch End
...
// Two mm play support speaker and line out
#if ENABLE_DUAL_OUTPUT
    } else if (outputDesc->isStrategyActive(STRATEGY_MEDIA_SUB)){ 
        device = getDeviceForStrategy(STRATEGY_MEDIA_SUB, fromCache);
#endif
// MStar Android Patch End
...
// Two mm play support speaker and line out
#if (ENABLE_DUALMONO || ENABLE_DUAL_OUTPUT)
    case AudioSystem::MUSIC_SUB:
#endif
#if ENABLE_DUAL_OUTPUT
        return STRATEGY_MEDIA_SUB;
#endif
#if ENABLE_DUALMONO
        return STRATEGY_MEDIA;
#endif
...
// Two mm play support speaker and line out
                  ...
                  ||isStreamActive(AudioSystem::MUSIC_SUB, SONIFICATION_RESPECTFUL_AFTER_MUSIC_DELAY)) {
#else
        } else if (isStreamActive(AudioSystem::MUSIC, SONIFICATION_RESPECTFUL_AFTER_MUSIC_DELAY)) {
#endif
// MStar Android Patch End
...
// Two mm play support speaker and line out
#if ENABLE_DUAL_OUTPUT
        else if (isStreamActive(AudioSystem::MUSIC_SUB, SONIFICATION_RESPECTFUL_AFTER_MUSIC_DELAY)) {
            device = getDeviceForStrategy(STRATEGY_MEDIA_SUB, false /*fromCache*/);
        }
#endif
// MStar Android Patch End
        else {
    ...
    case STRATEGY_MEDIA: {
        ...
        if (mForceUse[AudioSystem::FOR_MEDIA] == AudioSystem::FORCE_BT_MIC) {
        ...
        device |= device2;
        // MStar Android Patch Begin
        // Fixed TV audio output through BT
        char bt_driver[PROPERTY_VALUE_MAX] = {'\0'};
        if (property_get("mstar.bt.driver",bt_driver, NULL) > 0) {
            if (strategy==STRATEGY_MEDIA) {
                if (device2&AUDIO_DEVICE_OUT_ALL_A2DP) {
                    ALOGV("add outspeaker to STRATEGY_MEDIA when a2dp mode!!");
                    device &= ~device2;
...
// Two mm play support speaker and line out
#if ENABLE_DUAL_OUTPUT
    case STRATEGY_MEDIA_SUB: {
        ALOGV("STRATEGY_MEDIA_SUB: mAvailableOutputDevices %x",mAvailableOutputDevices);
        uint32_t device2 = AUDIO_DEVICE_NONE;
        if (strategy != STRATEGY_SONIFICATION) {
            // no sonification on remote submix (e.g. WFD)
            device2 = mAvailableOutputDevices & AUDIO_DEVICE_OUT_REMOTE_SUBMIX;
        ...
        if (device2 == AUDIO_DEVICE_NONE) {
            device2 = mAvailableOutputDevices & AUDIO_DEVICE_OUT_LINE_OUT;
        }
        if ((device2 == AUDIO_DEVICE_NONE) &&
                mHasA2dp && (mForceUse[AudioSystem::FOR_MEDIA] != AudioSystem::FORCE_NO_BT_A2DP) &&
                (getA2dpOutput() != 0) && !mA2dpSuspended) {
        ...
        // STRATEGY_ENFORCED_AUDIBLE, AUDIO_DEVICE_NONE otherwise
        device |= device2;
        ALOGV("device %x",device);
        if (device) break;
        device = mDefaultOutputDevice;
        if (device == AUDIO_DEVICE_NONE) {
            ALOGE("getDeviceForStrategy() no device found for STRATEGY_MEDIA_SUB");
        }
        break;
    }
#endif
...
audio_devices_t AudioPolicyManagerBase::getDeviceForInputSource(int inputSource)
{
    ...
    case AUDIO_SOURCE_BT_MIC:
        ...
        // MStar Android Patch End
        ALOGD("mAvailableInputDevices %x", mAvailableInputDevices);
        ALOGI("AUDIO_SOURCE_BT_MIC %s device:%x ", __FUNCTION__, device);
        break;
    // MStar Android Patch End
    ...
    case AUDIO_SOURCE_REMOTE_SUBMIX:
        ...
        break;
    // MStar Android Patch Begin
    // modify policy for dual audio recoder
    case AUDIO_SOURCE_CAPTURE_DEV0:
        if (mAvailableInputDevices & AUDIO_DEVICE_IN_CAPTURE_DEVICE0) {
            device = AUDIO_DEVICE_IN_CAPTURE_DEVICE0;
            break;
        }
    case AUDIO_SOURCE_CAPTURE_DEV1:
        if (mAvailableInputDevices & AUDIO_DEVICE_IN_CAPTURE_DEVICE1) {
            device = AUDIO_DEVICE_IN_CAPTURE_DEVICE1;
            break;
        }
    // MStar Android Patch End
    default:
...
// Two mm play support speaker and line out
#if ENABLE_DUAL_OUTPUT
        case AUDIO_DEVICE_OUT_LINE_OUT:
#endif
// MStar Android Patch End
        default:
...
// Two mm play support speaker and line out
#if ENABLE_DUAL_OUTPUT
        if (isStreamActive(AudioSystem::MUSIC_SUB, SONIFICATION_HEADSET_MUSIC_DELAY) ||
                mLimitRingtoneVolume) {
            audio_devices_t musicDevice = getDeviceForStrategy(STRATEGY_MEDIA_SUB, true /*fromCache*/);
            float musicVol = computeVolume(AudioSystem::MUSIC_SUB,
                               mStreams[AudioSystem::MUSIC_SUB].getVolumeIndex(musicDevice),
                               output,
                               musicDevice);
            float minVol = (musicVol > SONIFICATION_HEADSET_VOLUME_MIN) ?
                                musicVol : SONIFICATION_HEADSET_VOLUME_MIN;
...
const struct StringToEnum sDeviceNameToEnumTable[] = {
    ...
    // Two mm play support speaker and line out
#if ENABLE_DUAL_OUTPUT
    STRING_TO_ENUM(AUDIO_DEVICE_OUT_LINE_OUT),
#endif
    // MStar Android Patch End
    // MStar Android Patch Begin
    // proting from brcm
    ...
    // MStar Android Patch Begin
    // modify policy for dual audio recoder
    STRING_TO_ENUM(AUDIO_DEVICE_IN_CAPTURE_DEVICE0),
    STRING_TO_ENUM(AUDIO_DEVICE_IN_CAPTURE_DEVICE1),
    // MStar Android Patch End
...
// MStar Android Patch Begin
#if ENABLE_DUAL_OUTPUT
            // the line out device connect
            mAttachedOutputDevices = mAttachedOutputDevices | AUDIO_DEVICE_OUT_LINE_OUT;
#endif
// MStar Android Patch End
           ALOGW_IF(mAttachedOutputDevices == AUDIO_DEVICE_NONE,
            ...
            // MStar Android Patch Begin
            // Enable capture devices for dual audio recoder
            mAvailableInputDevices = mAvailableInputDevices | (AUDIO_DEVICE_IN_CAPTURE_DEVICE0 & ~AUDIO_DEVICE_BIT_IN);
            mAvailableInputDevices = mAvailableInputDevices | (AUDIO_DEVICE_IN_CAPTURE_DEVICE1 & ~AUDIO_DEVICE_BIT_IN);
            // MStar Android Patch Begin
